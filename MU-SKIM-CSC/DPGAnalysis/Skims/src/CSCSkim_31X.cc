...
//   10  = selection for magnetic field studies only based on global muons (not considering CSC STA + track)
//
//
//======================================================================

#include "DPGAnalysis/Skims/interface/CSCSkim_31X.h"
...
CSCSkim::filter(edm::Event& event, const edm::EventSetup& eventSetup)
{
  ...
  LogDebug("CSCSkim") << "Run: " << iRun << "\tEvent: " << iEvent << "\tn Analyzed: " << nEventsAnalyzed;
  ...
  //In 31X the global muons are still just a TrackCollection?
  ...
  //AF skim based only on Global muons
  if (typeOfSkim == 10) {
    event.getByLabel(GLBMuonTag,gMuons);
  }
  ...
  bool GoodForOctoberExe = false;
  if (typeOfSkim == 10) {
    GoodForOctoberExe = doOctoberExeSelection(gMuons);
    if (GoodForOctoberExe) {nEventsForOctoberExe++;}
  }
  ...
  if (typeOfSkim == 10) {selectThisEvent = GoodForOctoberExe;}
...
//---------------------------------
//  October Exercise : selection based only on global mu
//
//----------------------------------
bool CSCSkim::doOctoberExeSelection(edm::Handle<reco::TrackCollection> gMuons) {
  ...
  for (reco::TrackCollection::const_iterator global = gMuons->begin(); global != gMuons->end(); ++global ) {
    ...
    //      float pDef  = global->p();
      ...
      float redChiSq = global->normalizedChi2();
      ...
      int nDTHits = 0;
      int nCSCHits = 0;
      for (trackingRecHit_iterator hit = global->recHitsBegin(); hit != global->recHitsEnd(); ++hit ) {
        const DetId detId( (*hit)->geographicalId() );
        if (detId.det() == DetId::Muon) {
          if (detId.subdetId() == MuonSubdetId::DT) {
            nDTHits++;
  ...
  } // end loop over global muon collection
  //-----------------------------------
  // do we accept this event?
  //-----------------------------------

  acceptThisEvent = (nGoodGlobalMuons > 0) ;
...
bool CSCSkim::doBFieldStudySelection(edm::Handle<reco::TrackCollection> saMuons,  edm::Handle<reco::TrackCollection> tracks, edm::Handle<reco::TrackCollection> gMuons) {
